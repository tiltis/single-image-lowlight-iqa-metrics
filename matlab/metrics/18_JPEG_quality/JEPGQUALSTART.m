...
% 엑셀 파일 저장을 위한 변수
excel_file = 'JPEG_Quality_Scores.xlsx';  
header = [{'JPEG_Q'}, arrayfun(@num2str, 1:num_tests, 'UniformOutput', false)];  % 엑셀 헤더 설정
row_names = {'MSR', 'DCT', 'EF', 'FMMR', 'GRW', 'MITM'};  % 행 이름 설정
jpeg_scores_matrix = NaN(length(image_files), num_tests);  % 결과를 저장할 행렬

% 각 폴더에 대해 JPEG 품질 점수 계산
for i = 1:num_tests
    % 각 폴더 경로 설정
    ...
    for idx = 1:length(image_files)
        ...
        if ~isempty(file_info)
            ...
            % jpeg_quality_score 함수 호출하여 JPEG 품질 점수 계산
            jpeg_score = jpeg_quality_score(img);
            jpeg_scores_matrix(idx, i) = jpeg_score; % JPEG 품질 점수 저장
        else
...
% 엑셀 파일로 결과 저장
xlswrite(excel_file, [header; [row_names', num2cell(jpeg_scores_matrix)]]);  % 엑셀로 내보내기
