% 원하는 i 값을 설정 (예: 1, 2 등)
i = 1;  % MITM 폴더를 대상으로 합니다.

% 이미지가 저장된 폴더 경로 (실제 경로로 설정)
image_folder = 'C:/Users/PC00/Desktop/score/metrics/origtestresult/MITM'; 

% 처리할 이미지 파일 이름 리스트 (확장자 무관)
image_files = {'MITM1', 'MITM2', 'MITM3', 'MITM4', 'MITM5', 'MITM6', ...
               'MITM7', 'MITM8', 'MITM9', 'MITM10', 'MITM11', 'MITM12'}; 

% CEIQ 값을 저장할 배열
ceiq_scores = zeros(1, length(image_files));

% 폴더 내 모든 파일 정보 가져오기
all_files = dir(fullfile(image_folder, '*')); % 모든 파일 목록 가져오기
disp({all_files.name}); % 파일 이름 목록 출력

% 각 이미지 파일 불러오고 CEIQ 계산
for idx = 1:length(image_files)
    ...
    if ~isempty(file_info)
        ...
        % CEIQ 함수 호출하여 CEIQ 값 계산
        ceiq_score = CEIQ(img);
        ceiq_scores(idx) = ceiq_score; % CEIQ 점수 저장
    else
        fprintf('파일 %s를 찾을 수 없습니다.\n', file_pattern);
        ceiq_scores(idx) = NaN; % 파일이 없을 때 NaN 값으로 설정
    end
end

% 결과 출력
fprintf('CEIQ Scores for i=%d:\n', i);
fprintf('%.4f\n', ceiq_scores);

% 엑셀 파일로 결과 저장
excel_file = 'CEIQ_Scores_MITM.xlsx';  % 저장할 파일명
header = image_files;  % 데이터 헤더 설정
data = num2cell(ceiq_scores);  % CEIQ 스코어를 셀 배열로 변환
