...
% 이미지가 저장된 폴더 경로
image_folder = sprintf('C:/Users/PC00/Desktop/score/metrics/origtestresult/MITM', i); % 동적 경로 설정

% 처리할 이미지 파일 이름 리스트 (확장자 무관)
image_files = {'MITM'}; % 이미지 파일 이름 패턴 (MITM으로 시작하는 모든 파일)

% 메트릭 값을 저장할 배열
metrics = zeros(1, 12); % 파일 개수에 맞게 배열 크기 설정 

% 폴더 내 모든 파일 정보 가져오기
all_files = dir(fullfile(image_folder, '*.*')); % 모든 파일 검색
disp({all_files.name}); % 파일 이름 목록 출력

% 각 이미지 파일 불러오고 메트릭 계산
for idx = 1:12
    % 파일 이름 패턴 생성 (예: MITM1.*, MITM2.*, ...)
    file_pattern = sprintf('%s%d', image_files{1}, idx); 
    
    % 해당 패턴에 맞는 파일 찾기 (확장자 무관)
