...
% 엑셀 파일 저장을 위한 변수
excel_file = 'SSIM_Quality_Scores.xlsx';  
header_le = [{'SSIM(LE)'}, arrayfun(@num2str, 1:num_tests, 'UniformOutput', false)];  % 엑셀 헤더 설정
header_he = [{'SSIM(HE)'}, arrayfun(@num2str, 1:num_tests, 'UniformOutput', false)];  % 엑셀 헤더 설정
row_names = {'aMSR', 'DCT', 'EF', 'FMMR', 'GRW', 'MITM'};  % 행 이름 설정

% SSIM 값을 저장할 행렬
ssim_scores_le = NaN(length(row_names), num_tests);  
ssim_scores_he = NaN(length(row_names), num_tests);  

% 각 폴더에 대해 SSIM 품질 점수 계산
for i = 1:num_tests
    % 각 test 폴더 경로 설정
    ...
    % LE 폴더 내 모든 파일 정보 가져오기
    le_files = dir(fullfile(le_folder, '*.png'));
    ...
    for idx = 1:length(row_names)
        ...
        if isfile(file_path)
            ...
            [mssim, ~] = ssim_index(le_img, img, K, window, L);
            ssim_scores_le(idx, i) = mssim;
        else
    ...
    % HE 폴더 내 모든 파일 정보 가져오기
    he_files = dir(fullfile(he_folder, '*.png'));
    ...
    for idx = 1:length(row_names)
        ...
        if isfile(file_path)
            img = imread(file_path);
            [mssim, ~] = ssim_index(he_img, img, K, window, L);
            ssim_scores_he(idx, i) = mssim;
        else
...
% 엑셀 파일로 결과 저장
xlswrite(excel_file, [header_le; [row_names', num2cell(ssim_scores_le)]], 'Sheet1', 'A1');  % LE 결과 엑셀로 내보내기
xlswrite(excel_file, [header_he; [row_names', num2cell(ssim_scores_he)]], 'Sheet1', 'A8');  % HE 결과 엑셀로 내보내기
