...
% 결과를 저장할 엑셀 파일 이름 설정
output_excel_file = 'C:\opencv\4mon\scoreimg\CPBD_Scores.xlsx';


% CPBD 계산
metric_cpbd = zeros(1, numel(filtered_files)); % CPBD 결과를 저장할 배열

disp('Starting CPBD metric computation...');
...
for i = 1:numel(filtered_files)
    ...
    disp(['Processed: ', filtered_files(i).name, ' | CPBD Score: ', num2str(metric_cpbd(i))]);
end

disp('CPBD metric computation complete.');

% 결과 저장
output_data = cell(numel(filtered_files) + 1, 2);
output_data{1, 1} = 'Image Name';
output_data{1, 2} = 'CPBD Score';

for i = 1:numel(filtered_files)
    output_data{i + 1, 1} = filtered_files(i).name; % 이미지 이름
    output_data{i + 1, 2} = metric_cpbd(i);        % CPBD 점수
end
