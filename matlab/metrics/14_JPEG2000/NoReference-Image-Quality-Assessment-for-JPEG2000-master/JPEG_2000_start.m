...
% 입력 폴더 설정
image_path = 'C:\Users\PC00\Desktop\score\metrics\newtestresult\test2'; % 입력 폴더 경로
...
% 결과를 저장할 엑셀 파일 이름 설정
output_excel_file = 'JPEG2000_Scores.xlsx';

% JPEG2000 MOS 계산
metric_mos = zeros(1, numel(filtered_files)); % JPEG2000 MOS 결과를 저장할 배열

disp('Starting JPEG2000 MOS metric computation...');
...
for i = 1:numel(filtered_files)
    ...
    try
        ...
        warning(['Unable to process file: ', filtered_files(i).name, '. Skipping...']);
        metric_mos(i) = NaN; % 오류 발생 시 NaN 값 저장
        continue;
    end

    % jpeg_2000 함수 호출하여 MOS 계산
    try
        mos_score = jpeg_2000(A);
        metric_mos(i) = mos_score; % MOS 점수 저장
        disp(['Processed: ', filtered_files(i).name, ' | MOS Score: ', num2str(mos_score)]);
    catch ME
        warning(['Error computing MOS for file: ', filtered_files(i).name, '. Skipping...']);
        metric_mos(i) = NaN; % 오류 발생 시 NaN 값 저장
    end
end

disp('JPEG2000 MOS metric computation complete.');

% 결과 저장
output_data = cell(numel(filtered_files) + 1, 2);
output_data{1, 1} = 'Image Name';
output_data{1, 2} = 'MOS Score';

for i = 1:numel(filtered_files)
    output_data{i + 1, 1} = filtered_files(i).name; % 이미지 이름
    output_data{i + 1, 2} = metric_mos(i);         % MOS 점수
end
