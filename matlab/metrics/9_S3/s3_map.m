...
% 패딩이 이미지의 크기와 일치하도록 조정
[rows, cols] = size(img);
pad_L = fliplr(img(:, 1:pad_len)); % Take 16 columns on the left of the
                                    % original image to pad to the left
pad_R = fliplr(img(:, end-pad_len+1:end)); % Take 16 columns on the right of the
                                        % original image to pad to the right

% Ensure padding size matches the image size
pad_L = pad_L(:, 1:pad_len);
pad_R = pad_R(:, 1:pad_len);

img = [pad_L, img, pad_R]; %Pad left and right

pad_T = flipud(img(1:pad_len, :)); %Similarly, pad top and bottom
pad_B = flipud(img(end-pad_len+1:end, :));
img = [pad_T; img; pad_B];
...
% Remove padded parts
res = res(pad_len+1:end-pad_len, pad_len+1:end-pad_len);
end % function
...
pad_R = fliplr(img(:, end-pad_len+1:end));%Take pad_len columns on the right
